...
- **Herramienta usada:** asistente de IA generativa basado en LLM.
- **Tarea concreta:** cerrar la brecha entre lo que la guía del TP1 pide como mínimo (detallar solo el básico + un slice representativo por CU) y lo que un enfoque ágil real necesita para poder planificar sprints (una HU con criterios de aceptación por cada slice que va a ser implementado). Le pedimos a la IA que generara los criterios Given-When-Then para las seis HU faltantes: dos alternativas de `CU001` (cancelación manual, base de datos local no disponible), tres excepciones de `CU001` (parámetros fuera de rango, combinación incompatible, fallo del modo remoto) y una alternativa de `CU002` (resultado vacío tras filtros).
- **Discusión previa a pedirle a la IA:** el grupo se planteó primero si valía la pena. La guía dice explícitamente que "un slice se detalla como historia de usuario recién cuando algún TP posterior lo necesita" y que "no todo slice justifica ese nivel de inversión". Decidimos igual detallar todos porque:
  - En el `README` ya elegimos un modelo iterativo-incremental con prácticas ágiles, y en ágil la HU es la unidad mínima de sprint — la que efectivamente se planifica y se implementa. Dejar slices "nombrados sin criterios de aceptación" implica, al llegar al sprint correspondiente, no tener forma objetiva de decidir cuándo el trabajo del slice está terminado.
  - Los criterios Given-When-Then son la semilla natural de los casos de prueba de TP5 (Módulo 6). Escribirlos ahora, mientras la discusión de dominio está fresca, es más barato que reconstruirlos meses después.
  - Es más trabajo hoy, pero es piso concreto para lo que después hay que construir y probar.
- **Qué generó:**
  - Seis HU nuevas: `HU05_CU001_A1`, `HU06_CU001_A2`, `HU07_CU001_E2`, `HU08_CU001_E3`, `HU09_CU001_E4` y `HU10_CU002_A1`, cada una con dos criterios Given-When-Then.
  - Una actualización de la tabla de trazabilidad para que ningún slice quede como "*nombrada, sin detallar aún*" y una revisión de los textos introductorios de `casos-de-uso.md`, `historias-usuario.md` y `srs.md` para explicar la decisión de cobertura completa.
  - De paso, aprovechamos para corregir una redacción confusa en `HU02_CU001_B2`. El texto anterior terminaba con *"para poder seguir trabajando mientras se ejecuta y luego revisar los alineamientos sin cargar otra pantalla"*, y "otra pantalla" no tenía referente claro: si la tabla de resultados aparece al terminar, alguna vista es la que la carga; la palabra "otra" quedaba ambigua. Lo reescribimos como *"para poder seguir trabajando en la aplicación mientras la búsqueda corre, sin quedarme atado a una pantalla de espera"*, que es lo que originalmente queríamos decir: diferenciar el comportamiento asíncrono deseado del típico de la web oficial de NCBI (una página de espera que puede perderse si se cierra la pestaña).
- **Qué aceptamos:**
...
- **Qué modificamos:**
  - **`HU06_CU001_A2` (BD local no disponible).** La IA propuso un primer criterio donde el sistema "ofrecía al investigador la opción de cambiar a modo remoto contra una base equivalente de NCBI". Volvimos a insistir en el argumento que ya habíamos discutido en la Entrada 4: no existen equivalencias reales entre una base local del laboratorio y las bases estándar de NCBI, y proponer el cambio sería científicamente engañoso. La versión final del criterio incluye la palabra **"sin"** explícita: *"sin proponer un cambio automático a modo remoto"*, para dejar la decisión en manos del investigador.
  - **`HU08_CU001_E3` (combinación incompatible).** La IA propuso, en un primer borrador, un criterio del tipo "si el investigador elige una combinación no soportada, el sistema le sugiere la combinación más cercana automáticamente". Lo modificamos: el sistema debe **sugerir las opciones válidas** (indicar qué programas son compatibles con lo que el usuario ya cargó), pero **no** elegir por él. Es la misma decisión de fondo que en `A2`: el sistema no toma decisiones científicas por el investigador.
- **Qué descartamos:**
