...
3. El investigador selecciona la **base de datos** de una lista: si eligió modo local, aparecen las bases del catálogo del laboratorio (leídas de D1); si eligió modo remoto, las bases estándar de NCBI.
...
- **A3 · Base de datos local no disponible.** El investigador seleccionó modo local y una base que en ese momento no está lista en D1 (por ejemplo, se está actualizando desde P3). El sistema informa el estado y sugiere elegir otra base o cambiar a modo remoto.
- **A4 · Fallo del servicio remoto NCBI.** En modo remoto, NCBI no responde, devuelve error, o supera el timeout. El sistema informa el problema y ofrece reintentar o cambiar a modo local si hay una base equivalente disponible.
...
2. El administrador elige **Dar de alta una nueva base**.
3. El sistema le pide: **nombre visible** de la base, **tipo** (nucleótidos o proteínas) y **origen del archivo FASTA** (subir un archivo local, o indicar la URL de una fuente pública como SwissProt).
4. El administrador completa los datos y presiona **Crear base**.
...
7. Al terminar, el sistema agrega la base al catálogo con estado **Disponible**, la persiste en D1, y notifica al administrador que la base ya puede usarse en el CU-01.

**Postcondición:** La nueva base aparece en el catálogo y queda disponible para que los investigadores la seleccionen en el CU-01.

### Slices secundarios nombrados

- **A1 · FASTA inválido o inconsistente con el tipo declarado.** El sistema detecta que el archivo no es FASTA legible, o que su contenido no coincide con el tipo declarado (por ejemplo se subió FASTA de proteínas indicando "nucleótidos"). Informa el problema y no crea la base.
- **A2 · Actualización de una base existente.** El administrador sube una versión nueva del FASTA para una base ya presente en el catálogo; el sistema reconstruye los índices y actualiza la fecha, sin cambiar el nombre visible ni el resto de sus metadatos. Las búsquedas en curso sobre la versión anterior no se interrumpen.
- **A3 · Baja de una base.** El administrador quita una base del catálogo. El sistema pide confirmación explícita, libera los índices en D1 y deja constancia en el registro. Las búsquedas históricas que la usaron siguen visibles en el historial, marcadas con la nota de que la base ya no existe.
- **A4 · Falla la construcción del índice.** `makeblastdb` termina con error (falta de espacio, FASTA corrupto detectado a mitad, etc.). El sistema informa el error crudo y no agrega la base al catálogo.
