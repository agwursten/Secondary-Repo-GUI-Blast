...
- **Un CU no es una secuencia de pasos del actor, sino una capacidad discreta del sistema.** Por eso el proceso P1 dio lugar a **dos** casos de uso — `CU001` (ejecutar una búsqueda) y `CU002` (refinar y descargar resultados) — en vez de a un único CU largo. El investigador puede correr una búsqueda (`CU001`) una sola vez y luego usar `CU002` varias veces sobre esos mismos resultados (probar distintos filtros, distintos formatos) sin volver a correr BLAST.
